...
def prepare_torch():
    ...
    mat2 = Variable(torch.randn(4096, 1000), requires_grad=False)
    mat3 = Variable(torch.randn(1000, 1), requires_grad=False)
    if torch.cuda.is_available():
        print("Mat2 and Mat3 CUDA")
        mat2.cuda()
        mat3.cuda()
...
def prepare_tf():
    ...
    g_input = tf.placeholder(tf.float32, shape=(None, 4096))

    filter1 = tf.get_variable('weights1', [4096, 1000],
                              initializer=tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32),
                              dtype=tf.float32)

    filter2 = tf.get_variable('weights2', [1000,1],
                              initializer=tf.truncated_normal_initializer(stddev=3e-2, dtype=tf.float32),
                              dtype=tf.float32)
    g_output = tf.matmul(g_input, filter1)
    g_output = tf.matmul(g_output, filter2)
...
def generate_dummy_images(fw, batch_size, image_size):
    while True:
        images = np.random.randint(256, size=(batch_size, 4096))
        images = images / 255.0
