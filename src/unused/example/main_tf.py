...
def decode_image(image_file):
    return tf.image.decode_image(image_file, channels=3)


def prepare_images(file_names):
    return file_names


image_reader = tf.WholeFileReader()

img_files_val = tf.train.match_filenames_once("/Users/justas/PycharmProjects/ugproject/img/*.jpg")
img_files_queue = tf.train.string_input_producer(img_files_val, num_epochs=1, shuffle=False)
_, img_file = image_reader.read(img_files_queue)
image_decoded = decode_image(img_file)

image_decoded = tf.image.per_image_standardization(image_decoded)
image_decoded = tf.clip_by_value(image_decoded, -1.0, 1.0)

image_batch = tf.train.batch([image_decoded], batch_size=BATCH_SIZE, shapes=(224, 224, 3),
                             allow_smaller_final_batch=True)
alexnet_prob, parameters = alexnet_inference(image_batch)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        full_runs = IMG_COUNT // BATCH_SIZE
        extra_run = int(IMG_COUNT % BATCH_SIZE > 0)
        for i in range(full_runs + extra_run):
            p = sess.run(alexnet_prob)
            print(p.shape)
